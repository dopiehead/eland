...
$noofitem = (int) ($_POST['noofitem'] ?? 1);
$product_type = $conn->real_escape_string(isset($_POST['product_type']) && !empty($_POST['product_type']) ? $_POST['product_type'] : 'auction');
...
if ($itemId > 0 && $noofitem > 0 && !empty($seller_id) && !empty($seller_type) && !empty($buyer)) {
    ...
    if ($checkStmt->num_rows > 0) {
        ...
        $insertStmt = $conn->prepare("INSERT INTO cart (itemId, seller_id, product_type, noofItem, buyer, payment_status, date_added) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $insertStmt->bind_param("iisssis", $itemId, $seller_id, $product_type, $noofitem, $buyer, $payment_status, $date_added);
